...
public final class ActualizarPeriodoAcademicoValidation implements Validation<PeriodoAcademicoDTO> {

	private ActualizarPeriodoAcademicoValidation() {
		super();
	}

	public static Result validate(final PeriodoAcademicoDTO data) {
		return new ActualizarPeriodoAcademicoValidation().execute(data);
	}
	...
	public Result execute(final PeriodoAcademicoDTO data) {
		...
		if (UtilObject.isNull(data)) {
			result.addMessage(PeriodoAcademicoValidatorMessage.UPDATE_ESTUDIANTE_OBJECT_NULL_MESSAGE);
		} else {
