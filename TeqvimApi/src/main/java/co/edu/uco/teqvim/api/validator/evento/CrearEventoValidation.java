...
public class CrearEventoValidation implements Validation<EventoDTO> {
	
	private CrearEventoValidation() {
		super();
	}

	public static Result validate(final EventoDTO data) {
		return new CrearEventoValidation().execute(data);
	}
	...
	public Result execute(final EventoDTO data) {
		...
		if (UtilObject.isNull(data)) {
			result.addMessage("No es posible generar un evento con los datos vacíos");
		} else {
